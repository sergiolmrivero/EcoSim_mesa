...
class MyModel(Model):
    ...
    def __init__(self, n1, n2, f):
        ...
        self.economy = Economy(0, self, self.economy, self.initial_assets,
        ...
        for i in range(self.num_firms1):
            f1 = Firm1Agent(i, self, self.economy, self.initial_assets,
                            self.initial_liabilities, self.initial_cash, 
                            self.initial_inventory)
            self.schedule.add(f1)
            self.grid.place_agent(f1, (0, 1))
        for i in range(self.num_firms1):
            f2 = Firm2Agent(i, self, self.economy, self.initial_assets,
                            self.initial_liabilities, self.initial_cash, 
                            self.initial_inventory)
            self.schedule.add(f2)
            self.grid.place_agent(f2, (0, 2))
        for i in range(self.num_firms1):
            fm = FamilyAgent(i, self, self.economy, self.initial_assets,
                             self.initial_liabilities, self.initial_cash, 
                             self.initial_inventory)
